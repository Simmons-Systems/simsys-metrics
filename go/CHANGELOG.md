...
### Note on `simsys_runtime_gc_collections_total`

**Unchanged in Go** — still `{service}`, still `runtime.MemStats.NumGC`.
The Python lane moved to `simsys_runtime_gc_collections_by_generation_total`
(#50345), so this name now has exactly one meaning across the catalogue.


## [go/v1.0.0] — 2026-08-22
...
- **Published to the Go module proxy.** `go get github.com/Simmons-Systems/simsys-metrics/go@v1.0.0` — note that a v1 module path needs no `/vN` suffix; a future v2 will.
